...
- Özellikle **RCON / GameLabs / CFTools** gibi entegrasyonlar bu repodan kaldırılmıştır ve yeniden hedeflenmez.
...
- Çoklu sunucu (multi-instance) desteği, proje yapısını kökten etkilediği için roadmap'te **P1 önceliğine** alındı.
- Hedef: Tüm modüller (settings/server/mods/config/log) `X-Instance-Id` tabanlı instance bağlamıyla çalışır.
